...
class ProChats extends MY_Controller {
	...
	public function chat($id = null){
		if (!$this->ion_auth->logged_in()){
			...
			$chat_rooms = ChatRoom::find('all',array('conditions'=>array('professional_id = ?',$this->ion_auth->user()->row()->id), 'order' => 'updated_at desc'));
			...
			foreach ($chat_rooms as $row) {
				...
				$contacts[] = array(
					'id' => $row->id,
					'user' => User::find_by_id($row->customer_id)->first_name,
          			'price'		=> $row->price,
                	'created_at'	=> $row->created_at,
                	
                	'message'	=> Chat::last(array('conditions'=>array('chat_room_id = ?',$row->id),'select' => 'message, is_view_professional'))
                );
			...
			if(!empty($contacts)){
				...
				if($id!=null){
					...
					$user_details = ChatRoom::find_by_id($id);

					$this->data['request_status'] =  ResponseForProposal::find_by_id($user_details->request_id);
					$this->data['active_status'] = ResponseForProposal::find_by_id($user_details->request_id)->status;
					$this->data['price'] =  $user_details;
					$this->data['customer_name'] =  User::find_by_id($user_details->customer_id);
					...
					$user_details = ChatRoom::find_by_id($chat_rooms_id);
			...
			$this->template->load('frontend/front_base','professional/pro_chat',$this->data);
		}
	}
	...
	public function send_message(){
		if (!$this->ion_auth->logged_in()){
			...
			if($chat){
				$userid = '';
				$name = '';
				$u_type = UsersGroup::find_by_user_id($this->ion_auth->user()->row()->id)->group_id;
				
				if($u_type == '2'){
					$userid = $chatroom->professional_id;
					$name = User::find_by_id($chatroom->customer_id)->first_name;
				} else {
					$userid = $chatroom->customer_id;
					$name = User::find_by_id($chatroom->professional_id)->professionals->company_name;
				}

				$recipient = User::find_by_id($userid);
		...
		$this->form_validation->set_rules('message', 'Message', 'max_length[250]');
			if ($this->form_validation->run() == FALSE) {
		      ...
		      if (isset ($_FILES['media_file'])){ 
		        ...
		        print_r(json_encode(array("status" => "destination", "destination" => 'professional-messages/'.$this->input->post('chat_room_id'), 'message' => 'File uploaded successfully')));
		      }
		  }
	}
