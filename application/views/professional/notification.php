 ...
 <main id="content" role="main">
     ...
     <div class="container">
         ...
         <center>
             <a class="btn <?= ($this->uri->segment(1) == 'all-leads') ? 'btn-primary' : 'btn-default' ?>" href="<?= base_url('all-leads'); ?>">All Leads</a>

             <a class="btn <?= ($this->uri->segment(1) == 'today-leads') ? 'btn-primary' : 'btn-default' ?>" href="<?= base_url('today-leads'); ?>">Today Leads</a>

             <a class="btn <?= ($this->uri->segment(1) == 'yesterday-leads') ? 'btn-primary' : 'btn-default' ?>" href="<?= base_url('yesterday-leads'); ?>">Yesterday Leads</a>

             <a class="btn <?= ($this->uri->segment(1) == 'current-week-leads') ? 'btn-primary' : 'btn-default' ?>" href="<?= base_url('current-week-leads'); ?>">Current Week Leads</a>
         </center>
                             ...
                             <div class="card-body">
                                 ...
                                 <div>
                                     <p class="font-size-14">
                                         ...
                                         <?php if ($row->quote_request->status == 0 or $row->status == 0) { ?> <span class="badge badge-danger">REQUEST CANCELLED</span><?php } else if ($row->quote_request->status == 1) { ?> <span class="badge badge-success">ONGOING REQUEST</span><?php } else { ?><span class="badge badge-info">REQUEST COMPLETED</span><?php } ?> <?= ' | ' . $row->quote_request->cancel_request ?></p>
                                 </div>
                                 <hr>
                     ...
                     <form action="<?= base_url('professional/Notifications/send_quote') ?>" method="post" class="js-validate ajax-form" novalidate="novalidate">
                         <div class="row">
                             ...
                             <div class="col-sm-12 mb-2">
                                 <div class="js-form-message">
                                     <div class="js-focus-state input-group u-form">
                                         <input class="form-control u-form__input" value="<?= set_value('price') ? set_value('price') : "" ?>" onkeypress="if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;" name="price" required="" placeholder="Please write your price" aria-label="Please write your price" data-msg="Please write your price" data-error-class="u-has-error" data-success-class="u-has-success">
                                         <?= invalid_error('price') ?>
                                     </div>
                                 </div>
                             </div>
